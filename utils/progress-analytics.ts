...
class ProgressAnalyticsService {
  ...
  private calculateOverallScore(metrics: DetailedMetrics): number {
    ...
    Object.values(metrics).forEach((category) => {
      Object.values(category as Record<string, { current: number; target: number       }>).forEach((skill) => {
  ...
  private identifyStrongestArea(metrics: DetailedMetrics): string {
    ...
    Object.entries(metrics).forEach(([category, skills]) => {
      const scores = Object.values(skills as Record<string,
                      { current: number; target: number }>).map(
  ...
  private getNextMilestone(metrics: DetailedMetrics, age: number): string {
    ...
    Object.entries(metrics).forEach(([category, skills]) => {
      Object.entries(skills as Record<string, { current: number; target: number }>).forEach(([skill, data]) => {
...
export const generateReportSummary = (
  data: { studentInfo: { age: number }; summary: { overallScore: number }; insights: ProgressInsight[] }
): string => {
  const { studentInfo, summary, insights } = data;

  const strongInsights = insights.filter((i: ProgressInsight) => i.category === 'strength').length;
  const improvementInsights = insights.filter((i: ProgressInsight) => i.category === 'improvement').length;
